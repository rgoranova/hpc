...
int main(int argc, char **argv)
{
    ...
    FILE *outfile;
    outfile = fopen("output_hybrid.txt", "w");
    ...
    int i,j,tid,n,l,shift,sh,istart,ifinal;
    double elapsed_time;
    ...
    while (mpf_cmp(time,T)<0)
    {
         ...
         for (i = 0; i<N; i++)
         {
            ...
            #pragma omp master
            {
            mpf_set(sum[1],sum[pad*(Numt-(Numt+1)/2)+1]);
